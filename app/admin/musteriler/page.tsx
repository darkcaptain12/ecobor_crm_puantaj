...
export default async function AdminMusteriler({
  ...
  const { data: cropTypes } = await supabaseServer
    .from('customers')
    .select('crop_type')
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-bold text-eco-text">Müşteriler</h1>
        <Link href="/admin/musteriler/yeni" className="eco-btn-primary">
      ...
      <div className="bg-white rounded-xl p-4 shadow-card border border-eco-border">
        <form method="get" className="flex flex-wrap gap-3">
          ...
          />
          <select name="region" defaultValue={searchParams.region ?? ''} className="eco-select w-44">
            <option value="">Tüm Bölgeler</option>
            {uniqueRegions.map((r: any) => (
              <option key={r} value={r}>{r}</option>
            ))}
          </select>
          <select name="crop_type" defaultValue={searchParams.crop_type ?? ''} className="eco-select w-44">
          ...
          <Link href="/admin/musteriler" className="eco-btn-secondary px-4 py-2">Temizle</Link>
        </form>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-card border border-eco-border overflow-hidden">
        <div className="overflow-x-auto">
              ...
              {customers?.map((c: any) => {
                ...
                return (
                  <tr key={c.id} className="hover:bg-eco-bg/50">
                    ...
                    <td className="px-4 py-3 text-eco-text-2 flex items-center gap-1">
                    ...
                    <td className="px-4 py-3">
                      <Link href={`/admin/musteriler/${c.id}`} className="eco-btn-secondary text-xs px-3 py-1">
                        Detay
                      </Link>
                    </td>
                  </tr>
          ...
          {!customers?.length && (
            <p className="text-center text-eco-gray py-12">Müşteri bulunamadı</p>
          )}
        </div>
      </div>
    </div>
